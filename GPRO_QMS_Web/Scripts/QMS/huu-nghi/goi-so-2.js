...
GPRO.namespace('GoiSo2');
GPRO.GoiSo2 = function () {
    var Global = {
        UrlAction: {
    ...
    function Get() {
        $.ajax({
            ...
            type: 'POST',
            data: JSON.stringify({ 'GetKL': false, 'counters': $('#counterid').val() }), 
            ...
            success: function (data) {
                ...
                if ($('.num-stt-ketluan').html() == obj.STTDangKham.toString()) {
                    $('.num-stt-ketluan').addClass('doi');
                    var inter = setInterval(function () {
                        $('.num-stt-ketluan').removeClass('doi');
                        clearInterval(inter);
                    }, 5000);
                }
