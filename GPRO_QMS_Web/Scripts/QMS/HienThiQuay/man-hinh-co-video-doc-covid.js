...
GPRO.BenhVien = function () {
    ...
    function SaveConfig() {
        var obj = {
            ...
            Row1_Css: $('#css_r1').val(),
            Row1_NumberCss: $('#css_r1_number').val(),
            Row2_Css: $('#css_r2').val(),
            Row2_NumberCss: $('#css_r2_number').val(),
            Adv_Css: $('#run_css').val(),
            Row: $('#row').val(),
            Col: $('#column').val(),
            Services: '',
        ...
        $.ajax({
            ...
            type: 'POST',
            data: JSON.stringify({ 'configStr': str, 'pageType': 7 }),
    ...
    function Get() {
        $.ajax({
            ...
            data: JSON.stringify({
                'counters': ('1,2,3,4,5,6'), 'services': ('1,2,3,4,5,6,7,8,9'), 'userId': 1, 'getLastFiveNumbers':true }),
            contentType: 'application/json charset=utf-8',
            success: function (data) {
    ...
    function GetConfig() {
        $.ajax({
            ...
            type: 'POST',
            data: JSON.stringify({ 'pageType': '7' }),
            contentType: 'application/json charset=utf-8',
            success: function (data) {
                ...
                if (obj != null) {
                    ...
                    css += '.row .cell {' + obj.Row1_Css + '  }';
                    css += '.row .cell .number{' + obj.Row1_NumberCss + '  }';
                    css += '.row-color .cell {' + obj.Row2_Css + ' }';
                    css += '.row-color .cell .number{' + obj.Row2_NumberCss + ' }';
                    ...
                    $('#css_r1_number').val(obj.Row1_NumberCss);
                    $('#css_r2').val(obj.Row2_Css);
                    $('#css_r2_number').val(obj.Row2_NumberCss);
        ...
        if (objs.length > 0) {
            ...
            for (var i = 0; i < Global.Data.rows; i++) {
                ...
                for (var z = 1; z <= col; z++) {
                    ...
                    str += `<div class="col m${(12 / (col * 2))} s${(12 / (col * 2))} cell" tk_${count}><div class="number">${obj1.tk}</div></div> 
                            <div class="col m${(12 / (col * 2))} s${(12 / (col * 2))} cell ${(z == col ? 'cell-last' : '')}" room_${count}>${obj1.room}</div>`;
                    count++;
    ...
    function BindTable(objs) {
        $.each(objs, function (i, item) {
            $('[tk_' + i + ']').html(`<div class="number">${objs[i].TicketNumber}</div>`);
